...
function App() {
	...
	const handleChangeRatioInput: React.ChangeEventHandler<HTMLInputElement> = useCallback((e) => {
		const possibleKeys = Object.keys(values)
		if (!possibleKeys.includes(e.target.name)) {
			throw new Error(`<input /> must have a name attribute that is one of the following: "${possibleKeys.join("\", \"")}"`)
		}
		...
		<>
			...
			<div>
				<div className="ratios-container">
					...
					<img src={showEqualSign ? equalSign : notEqualSign} alt={showEqualSign ? "equal sign" : "not equal sign"} />
					<div className="ratio-pair">
						<RatioInput
							value={values.c}
